...
extern "C"  int  MCF_Solve( int i,char *InFile, char *OutFile,MCF_network_t net);
         ...
         for(int r=1;r<=NoRecTable2;r++)
         {
            ...
            //  if (DT1 < 0) DT1 = 0;
             ...
             if( DT1 >= 0 )
             {
                ...
                if(net->max_cost < DT1) net->max_cost = DT1;
                arc->nextout = arc->tail->firstout;
                arc->tail->firstout = arc;
                arc->nextin = arc->head->firstin;
                arc->head->firstin =  arc;
                ...
                *(CostAGVToC+(i1-1)*Number_Container_Job+ RecTable2-1) =  DT1;
                ...
                if(net->max_cost < DT2) net->max_cost = DT2;
                arc->nextout = arc->tail->firstout;
                arc->tail->firstout = arc;
                arc->nextin = arc->head->firstin;
                arc->head->firstin =  arc;
...
void __fastcall TMCFAlgorithmForm::Timer1Timer(TObject *Sender)
{
  ...
  //   Timer3->Enabled = false;
     ...
     arc = Append_Arcs_From_AGVs_Nodes_To_Container_Nodes(Number_Of_AGVs,&Number_Of_ARCs,              //SF
                                    PortNameStr,TimeAGVToC,Number_Of_Container_Jobs,CostAGVToC,fp,0,&net,arc);
     ...
     arc = Append_Arcs_From_CPNodes_To_Any_Other_CNodes(Number_Of_AGVs,Number_Of_Container_Jobs,
                                &Number_Of_ARCs,TimeCToC,CostCToC,PortNameStr,fp,0,&net,arc);
                                                                                //SF
     ...
     int stat = MCF_Solve( 2,"temp.inp","temp.out",net); // Dynamic Fashion
     switch(stat)
     {
        case -1 :
...
//     register long i;
     ...
     */
     ...
     arc = net.arcs;
     arc = Append_Arcs_From_AGVs_Nodes_To_Container_Nodes(Number_Of_AGVs,&Number_Of_ARCs,              //SF
                                    PortNameStr,TimeAGVToC,Number_Of_Container_Jobs,CostAGVToC,fp,1,&net,arc);
     ...
     arc = Append_Arcs_From_CPNodes_To_Any_Other_CNodes(Number_Of_AGVs,Number_Of_Container_Jobs,
                                                 &Number_Of_ARCs,TimeCToC,CostCToC,PortNameStr,fp,1,&net,arc);
                                                                                             // SF
     ...
     int stat = MCF_Solve( 2,"temp.inp","temp.out",net); // Static Fashion
     switch(stat)
     {
        case -1 :
