...
double MCF_primal_obj( MCF_network_p net )
{
    ...
    for( node = net->nodes, node++; node != stop; node++ )
    {
        ...
        if(node->basic_arc->ident == MCF_DELETED) continue; // DNSA
...
{
#ifdef _WIN32
    // struct _timeb now;  // Commented by H.Rashidi
    struct timeb now;     // Added by H.Rashidi

    // _ftime(&now); // Commented by H.Rashidi
    ftime(&now);     // Added by H.Rashidi
